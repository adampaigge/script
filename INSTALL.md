...
The easiest way to install the latest version of the Script Core software is
by to download the latest precompiled binaries for your platform from the
[release page](https://github.com/adampaigge/script/releases). Currently,
...
It is possible to build your own copy of Script Core with the exact, tested,
dependencies, as used for the binary releases, by using the
[depends system](depends/description.md). Please refer to the
[depends README](depends/README.md) for instructions to build Script using
these dependencies.

### Compiling using system-provided libraries

  The following are developer notes on how to build Script on your native
  ...
  - [macOS Build Notes](doc/Building-Script-for-Mac.md)
...
Run with the `-testnet` option to run with "play script" on the test network, if you
...
Script Core is a multithreaded application, and deadlocks or other multithreading bugs
